...
class ppdb_char_lstm_model(object):
	...
	if params.nntype == 'lstm_char1':
            l_word_representation = word_representation_layer1([l_emb, l_char_emb, l_char_mask]) #lasagne.nonlinearities.tanh
	elif params.nntype == 'lstm_char2':
	    l_word_representation = word_representation_layer2([l_emb, l_char_emb, l_char_mask])
	elif params.nntype == 'lstm_char3':
            l_word_representation = word_representation_layer3([l_emb, l_char_emb, l_char_mask])
	else:
	    print 'something wrong in ppdb_char_lstm_model !'

        words_embg1 = lasagne.layers.get_output(l_word_representation, {l_in: g1batchindices, l_char_in: char_g1batchindices, l_char_mask: char_g1mask})
        self.word_representation_function = theano.function([g1batchindices, char_g1batchindices, char_g1mask], words_embg1)

        l_lstm = lasagne.layers.LSTMLayer(l_word_representation, params.layersize, peepholes=params.peephole,
                                          learn_init=False,
                                          mask_input=l_mask)
        l_average = lasagne.layers.SliceLayer(l_lstm, -1, 1)
        #l_matrix = lasagne_matrix_layer(l_word_representation, num_units=params.layersize, nonlinearity=params.nonlinearity, W = lasagne.init.Normal(0.01))
        #l_average = lasagne_average_layer([l_matrix, l_mask])

        embg1 = lasagne.layers.get_output(l_average, {l_in:g1batchindices, l_mask:g1mask, l_char_in:char_g1batchindices, l_char_mask:char_g1mask})
        embg2 = lasagne.layers.get_output(l_average, {l_in:g2batchindices, l_mask:g2mask, l_char_in:char_g2batchindices, l_char_mask:char_g2mask})
        embp1 = lasagne.layers.get_output(l_average, {l_in:p1batchindices, l_mask:p1mask, l_char_in:char_p1batchindices, l_char_mask:char_p1mask})
        embp2 = lasagne.layers.get_output(l_average, {l_in:p2batchindices, l_mask:p2mask, l_char_in:char_p2batchindices, l_char_mask:char_p2mask})
        ...
        network_params = lasagne.layers.get_all_params(l_average, trainable=True)

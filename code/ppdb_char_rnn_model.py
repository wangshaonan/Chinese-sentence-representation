...
class ppdb_char_rnn_model(object):
	...
	l_rnn = None
        if params.add_rnn:
            ...
            l_rnn = lasagne.layers.RecurrentLayer(l_word_representation, params.layersize, W_in_to_hid=identity_mat,
                                                  W_hid_to_hid=identity_mat, nonlinearity=lasagne.nonlinearities.linear,
                                                  learn_init=False, mask_input=l_mask)
        else:
            l_rnn = lasagne.layers.RecurrentLayer(l_word_representation, params.layersize, nonlinearity=params.nonlinearity,
                                                  learn_init=False, mask_input=l_mask)

        l_average = lasagne.layers.SliceLayer(l_rnn, -1, 1)

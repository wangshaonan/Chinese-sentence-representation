...
from ppdb_char_word_model import ppdb_char_word_model
from ppdb_char_rnn_model import ppdb_char_rnn_model
from ppdb_char_matrix_model import ppdb_char_matrix_model
from ppdb_char_dan_model import ppdb_char_dan_model
from ppdb_char_lstm_model import ppdb_char_lstm_model
import lasagne
import random
import numpy as np
import sys
...
parser.add_argument("-LWC", help="Lambda for word embeddings (normal training).", type=float)
...
parser.add_argument("-charfile", help="char embedding file")
...
parser.add_argument("-nonlinearity", help="Type of nonlinearity in Matrix and DAN model.",
                    type=int)
...
(chars, Wc) = getWordmap(params.charfile)
examples = getPPDBData(params.train, words, chars)
...
if params.nntype[0:-1] == 'word_char':
    model = ppdb_char_word_model(We, Wc, params)
elif params.nntype[0:-1] == 'rnn_char':
    model = ppdb_char_rnn_model(We, Wc, params)
elif params.nntype[0:-1] == 'matrix_char':
    model = ppdb_char_matrix_model(We, Wc, params)
elif params.nntype[0:-1] == 'dan_char':
    model = ppdb_char_dan_model(We, Wc, params)
elif params.nntype[0:-1] == 'lstm_char':
    model = ppdb_char_lstm_model(We, Wc, params)
else:
    "Error no type specified"

ppdb_utils.train(model, examples, words, chars, params)
